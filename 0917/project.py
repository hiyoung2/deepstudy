...
def build_model(drop=0.1, optimizer='adam', learning_rate=0.1, activation='relu'):
    inputs = tf.keras.Input(shape=(224,224), name='inputs')
    x = tf.keras.layers.Conv1D(32, kernel_size=(3, 3), padding='same', activation=activation)(inputs)
    x = tf.keras.layers.Dropout(drop)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2,2), strides=2)(x)
    x = tf.keras.layers.Conv1D(64, kernel_size=(3,3), padding='same', activation=activation)(x)
    x = tf.keras.layers.Dropout(drop)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2,2), strides=2)(x)
    x = tf.keras.layers.Conv1D(128, kernel_size=(3,3), padding='same', activation=activation)(x)
    x = tf.keras.layers.Conv1D(256, kernel_size=(3,3), padding='same', activation=activation)(x)
    x = tf.keras.layers.Dropout(drop)
